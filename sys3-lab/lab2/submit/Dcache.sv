...
module Dcache #(
    ...
    // debug 4: 如果是 mmio 访问，那么 ren_mem 信号就为 1，而 ren_mem 又是流水线的 stall 条件，导致传不到 WB 阶段
    // 所以 ren_mem 就会一直为 1，即使 mmio 访问已经完成了，导致后续的指令无法继续执行
    // 解决方法：ren_mem 信号在 mmio 访问时，只有在 rvalid_in 为 0 时才为 1，这样当读完内存之后，ren_mem 就会变为 0，流水线就可以继续执行，wen_mem 信号同理
    assign ren_mem = is_mmio ? (ren_cpu & !rvalid_in) : (is_mem ? (ren_dcache && !hit_dcache) : 0);
    assign raddr_out = is_mmio ? addr_cpu : (is_mem ? dcache_read_addr_mem : 0);

    assign wen_mem = is_mmio ? (wen_cpu & !wvalid_in) : wen_dcache;
